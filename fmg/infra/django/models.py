...
from fmg.domain.enums import EventRelevance, EventType, RunStatus
...
class ModelVersionORM(models.Model):
    ...
    model = models.ForeignKey(
        FinancialModelORM, on_delete=models.PROTECT, db_column="model_id", related_name="versions"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["model_id", "version"], name="unique_model_version")
        ]
        indexes = [
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["model_id", "parameter_version"], name="unique_parameter_version"
            ),
...
class ModelRunORM(models.Model):
    ...
    model = models.ForeignKey(
        FinancialModelORM, on_delete=models.PROTECT, db_column="model_id", related_name="runs"
    )

    model_version = models.ForeignKey(
        ModelVersionORM, on_delete=models.PROTECT, db_column="model_version_id", related_name="runs"
    )
    ...
    status = models.CharField(max_length=50, choices=[(s.value, s.value) for s in RunStatus])
...
class AnyLogORM(models.Model):
    ...
    event_type = models.CharField(max_length=50, choices=[(e.value, e.value) for e in EventType])

    author = models.CharField(max_length=150, default="SYSTEM")

    details = models.JSONField(encoder=DjangoJSONEncoder, default=dict)

    relevance = models.CharField(
        max_length=20, choices=[(r.value, r.value) for r in EventRelevance]
    )
