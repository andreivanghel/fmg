...
class DjangoParametersRepository(IParametersRepository):
    def get(self, parameter_set_id: int) -> ParameterSet:
        ...
        except ParameterVersionORM.DoesNotExist as e:
            raise ParametersNotFoundError(f"Parameter set {parameter_set_id} not found.") from e
    ...
    def save(self, parameter_set: ParameterSet) -> None:
        ...
        if parameter_set.parameter_version_id is None:
            raise MissingParamsSetIdError("Cannot save a parameter set that does not have an ID.")
    ...
    def save_if_status(self, parameter_set: ParameterSet, expected_status: bool) -> bool:
        ...
        try:
            updated = ParameterVersionORM.objects.filter(
                pk=parameter_set.parameter_version_id, approved=expected_status
            ).update(**updates)
    ...
    def create(self, parameter_set: ParameterSet) -> int:
        ...
        try:
            ...
            raise DatabaseError(f"Integrity error while creating parameter set: {e}") from e

        except OperationalError as e:
            # Infrastructure-level error
            raise DatabaseError(f"Database error while creating parameter set: {e}") from e
